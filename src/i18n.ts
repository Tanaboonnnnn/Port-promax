...
const thaiHoldingText: Record<string, Partial<(typeof holdings)[number]>> = {
  ...
  V: {
    thesis:
      "Visa เป็นเครือข่ายชำระเงินที่ใช้ทุนไม่หนัก แต่ได้ประโยชน์จากเงินสดที่ค่อย ๆ ถูกแทนด้วยดิจิทัล งาน OpenAI, token, stablecoin และ agentic commerce เป็นบวกเชิงกลยุทธ์ แต่ยังไม่ใช่หลักฐานกำไรที่ใช้เพิ่มน้ำหนักได้. เช็กพอยต์ 19 มิ.ย. ไม่พบหลักฐานใหม่ที่เปลี่ยน thesis.",
    killCondition:
  ...
  GOOGL: {
    thesis:
      "Alphabet ยังมีสินทรัพย์ชั้นดีอย่าง Search, YouTube, Cloud และเงินสด แต่คำถามใหญ่ยังเหมือนเดิม: capex และโครงสร้างเงินทุนด้าน AI จะคืนกลับมาเป็นมูลค่าต่อหุ้นได้แค่ไหน จึงยังถือ แต่ให้อยู่ระหว่างรีวิว. เช็กพอยต์ 19 มิ.ย. ไม่พบหลักฐานใหม่ที่ทำให้ thesis เสียหรือทำให้หลุดจากสถานะ under review.",
    killCondition:
  ...
  USD: {
    company: "เงินสด",
    thesis:
      "เงินสดเป็นตำแหน่งที่ตั้งใจถือ ไม่ใช่ช่องว่างในพอร์ต เช็กพอยต์ 19 มิ.ย. ยังไม่มีไอเดียใหม่ที่ชนะทั้งหุ้นเดิมและความยืดหยุ่นของเงินสดได้ชัดพอ.",
    killCondition:
...
const thaiDecisionText: Record<string, { subject: string; reason: string }> = {
  ...
  "2026-06-18 14:30 ET": {
    subject: "สแกนระหว่างวัน",
    reason:
      "สแกน 14:30 ET ยังไม่พบหลักฐานรายบริษัทที่ต้องเปลี่ยน thesis หรือน้ำหนักพอร์ต SearchLeak / CVE-2026-42824 ของ Microsoft เป็นเรื่องที่ต้องตามด้านความเชื่อมั่นต่อ enterprise AI แต่ยังไม่ใช่ thesis break. Visa, S&P Global และเงินสดยังถือได้เหมือนเดิม Alphabet ยังอยู่ระหว่างรีวิว AutoZone ยังเป็น watchlist ด้าน capital allocation ส่วน Copart ยังต่ำกว่าเกณฑ์ซื้อ.",
  },
};
...
const thaiWatchlist: Record<string, { whyInteresting: string; whyNotYet: string }> = {
  AZO: {
    ...
    whyNotYet:
      "ข่าวซื้อคืนเป็นบวก แต่เช็กพอยต์ 19 มิ.ย. ยังไม่มีหลักฐานใหม่ที่ทำให้ AutoZone ดีกว่าเงินสดหรือหุ้นแกนพอร์ต ต้องเห็นกำไร กระแสเงินสด leverage และ valuation หนุนชัดกว่านี้ก่อน.",
  },
  CPRT: {
    whyInteresting: "Copart มีเครือข่ายประมูลซากรถที่เฉพาะทาง ฐานผู้ซื้อกว้าง และโมเดล marketplace ที่ทนกว่าธุรกิจรถทั่วไป.",
    whyNotYet: "เช็กพอยต์ 19 มิ.ย. ไม่พบหลักฐานใหม่ สัญญาณฟื้นตัวยังไม่ชัดพอเมื่อเทียบกับโอกาสที่พอร์ตถืออยู่แล้ว.",
  },
};
...
export function getPortfolioContent(locale: Locale) {
  if (locale === "th") {
    return {
      portfolioSnapshot: {
        ...portfolioSnapshot,
        asOf: "เช็กพอยต์วันตลาดปิด 2026-06-19 09:30 ET; NAV ยืนยันล่าสุดจากราคาปิด 2026-06-12",
        mandate:
        ...
        outcomeQuality:
          "มูลค่าล่าสุดยังอิงราคาปิด 12 มิ.ย. เช็กพอยต์ 19 มิ.ย. ตรงกับ Juneteenth ซึ่งตลาดหุ้นสหรัฐฯ ปิด จึงไม่มีราคา intraday ให้ประเมิน และไม่พบหลักฐานรายบริษัทที่ต้องปรับพอร์ต.",
      },
      ...
      quarterlyStatus: [
        ...
        { label: "มีการปรับพอร์ตไหม", value: "ไม่มี หลังเช็กพอยต์วันตลาดปิด 19 มิ.ย. Microsoft, Visa, S&P Global, Alphabet และเงินสดยังน้ำหนักเดิม." },
