...
const thaiHoldingText: Record<string, Partial<(typeof holdings)[number]>> = {
  MSFT: {
    thesis:
      "Microsoft ยังเป็นแกนพอร์ตที่อธิบายได้ง่าย: ซอฟต์แวร์องค์กรเหนียวแน่น Azure ยังมี runway และ AI มีโอกาสเพิ่มมูลค่าให้ฐานลูกค้าเดิม ข้อตกลงไฟฟ้า 20 ปีของ Chevron/Project Kilby สำหรับดาต้าเซ็นเตอร์ใน West Texas เป็นหลักฐานว่าบริษัทจริงจังกับการล็อกไฟฟ้าสำหรับ AI แต่ก็ทำให้ต้องตาม capex พลังงาน เวลาเริ่มจ่ายไฟ และผลตอบแทนต่อหุ้นใกล้ขึ้น รอบสแกน 11:30 ET ยังไม่พบข่าวใหม่ที่เปลี่ยน thesis หรือน้ำหนักลงทุน.",
    killCondition:
  ...
  V: {
    thesis:
      "Visa เป็นเครือข่ายชำระเงินที่ใช้ทุนไม่หนัก แต่ได้ประโยชน์จากเงินสดที่ค่อย ๆ ถูกแทนด้วยดิจิทัล งาน OpenAI, token, stablecoin และ agentic commerce เป็นบวกเชิงกลยุทธ์ แต่ยังไม่ใช่หลักฐานกำไรที่ใช้เพิ่มน้ำหนักได้ รอบสแกน 11:30 ET ยังไม่พบข้อมูลใหม่ที่กระทบคุณภาพเครือข่ายหรือ economics ของธุรกิจ.",
  ...
  GOOGL: {
    thesis:
      "Alphabet ยังมีสินทรัพย์ชั้นดีอย่าง Search, YouTube, Cloud และเงินสด แต่คำถามใหญ่ยังเหมือนเดิม: capex และโครงสร้างเงินทุนด้าน AI จะคืนกลับมาเป็นมูลค่าต่อหุ้นได้แค่ไหน รอบสแกน 11:30 ET ยังไม่พอถอดสถานะ under review เพราะภาระลงทุน AI และ dilution ยังเป็นคำถามหลัก.",
  ...
  USD: {
    company: "เงินสด",
    thesis: "เงินสดเป็นตำแหน่งที่ตั้งใจถือ ไม่ใช่ช่องว่างในพอร์ต AutoZone มีข่าวซื้อคืนหุ้นที่ช่วยให้กรณีน่าดูขึ้น แต่รอบสแกน 11:30 ET ยังไม่มีไอเดียใหม่ที่ชนะทั้งหุ้นเดิมและความยืดหยุ่นของเงินสดได้ชัดพอ.",
...
const thaiWatchlist: Record<string, { whyInteresting: string; whyNotYet: string }> = {
  AZO: {
    ...
    whyNotYet: "ข่าวซื้อคืนเป็นบวก แต่รอบสแกน 11:30 ET ยังไม่พอให้ AutoZone ดีกว่าเงินสดหรือหุ้นแกนพอร์ต ต้องเห็นกำไร กระแสเงินสด leverage และ valuation หนุนชัดกว่านี้ก่อน.",
  },
  CPRT: {
    whyInteresting: "Copart มีเครือข่ายประมูลซากรถที่เฉพาะทาง ฐานผู้ซื้อกว้าง และโมเดล marketplace ที่ทนกว่าธุรกิจรถทั่วไป.",
    whyNotYet: "สัญญาณฟื้นตัวยังไม่ชัดพอเมื่อเทียบกับโอกาสที่พอร์ตถืออยู่แล้ว รอบสแกน 11:30 ET จึงยังเป็น watchlist ไม่ใช่ตำแหน่งลงทุน.",
  },
};
  ...
  if (locale === "th") {
    return {
      portfolioSnapshot: {
        ...
        outcomeQuality: "รอบสแกน 11:30 ET ไม่ได้เพิ่มจุดผลตอบแทนใหม่ หลังเช็กข้อมูลต่อจาก 10:54 ET ยังไม่มีหลักฐานใหม่ที่เปลี่ยนพอร์ต ข้อตกลงไฟฟ้า Chevron/Project Kilby ยังเป็นรายการติดตามด้าน AI infrastructure ของ Microsoft ไม่ใช่เหตุผลปรับน้ำหนัก.",
      },
      ...
      quarterlyStatus: [
        ...
        { label: "มีการปรับพอร์ตไหม", value: "ไม่มี หลังสแกน 2026-06-22 11:30 ET Microsoft, Visa, S&P Global, Alphabet และเงินสดยังน้ำหนักเดิม." },
...
function translateDecision(date: string) {
  if (date === "2026-06-22 11:30 ET") {
    return {
      subject: "สแกนระหว่างตลาด",
      reason:
        "รอบสแกน 11:30 ET เช็กข้อมูลใหม่หลัง 10:54 ET แล้วไม่พบหลักฐานที่ต้องเปลี่ยนพอร์ต ประเด็นสำคัญของเช้านี้ยังเป็นข้อตกลงไฟฟ้า 20 ปีของ Chevron/Project Kilby สำหรับดาต้าเซ็นเตอร์ Microsoft ใน West Texas ซึ่งช่วยยืนยันว่า Microsoft กำลังล็อกทรัพยากรสำคัญสำหรับ AI แต่ก็ทำให้ต้องตาม capex, พลังงาน, execution และผลตอบแทนจาก capacity ใกล้ขึ้น ยังไม่ใช่สัญญาณซื้อขายหรือเหตุผลเปลี่ยนน้ำหนัก Visa, S&P Global, Alphabet, AutoZone, Copart และเงินสดไม่มีหลักฐานใหม่ที่เปลี่ยน thesis และไม่มีการเพิ่มจุด benchmark ระหว่างวัน.",
    };
  }
